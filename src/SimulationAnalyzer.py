...
class SimulationAnalyzer:
    ...
    def agv_status_analyzer(self):
        ...
        for k in range(self.number_of_agvs):
            ...
            busy_percentage = np.round((busy / self.simulation_duration) * 100, 2)
            charging = sum(self.agv_charge[k])
            charging_percentage = np.round((charging / self.simulation_duration) * 100, 2)
            idle = sum(self.agv_idle[k])
            idle_percentage = np.round((idle / self.simulation_duration) * 100, 2)
            print('\nAGV ' + str(k + 1) + ':')
